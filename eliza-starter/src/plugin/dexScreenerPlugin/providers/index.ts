...
export class TokenPriceProvider implements Provider {
    async get(runtime: IAgentRuntime, message: Memory, state?: State): Promise<string> {
        try {
            ...
            const tokenPatterns = [
                /0x[a-fA-F0-9]{40}/, // Ethereum, Base, BNB, Avalanche, Polygon, etc.
                /[1-9A-HJ-NP-Za-km-z]{32,44}/, // SOL
                /41[a-fA-F0-9]{40}/, // Tron
                /[a-zA-Z0-9]+\.[a-zA-Z0-9]+\.[a-zA-Z0-9]+/, // ICP, Near
                /[0-9a-fA-F]{64}::[a-zA-Z0-9_:]+/, // Sui, Aptos
                /[a-zA-Z0-9]+\.[a-zA-Z0-9]+\.[a-zA-Z0-9]+/, // MultiverseX
                /[0-9a-fA-F]{40}\.[a-zA-Z0-9_]+/, // XRPL
                /[a-zA-Z0-9]+-[0-9a-z]+/, // MultiverseX tokens
            ];
    

            // Make API request
            const isAddress = tokenPatterns.some((pattern) => pattern.test(tokenIdentifier));
